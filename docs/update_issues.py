...
def get_issues():
    cmd = [GH_PATH, "issue", "list", "--limit", "150", "--label", "contribution-ready", "--json", "number,title,labels"]
...
4. **Key Implementation Steps**:
   ...
   - Ensure that the new changes decoupled where possible, preventing monolithic dependencies.
...
- [ ] **Core Logic**: Successfully implement `{task_focus}` as outlined in the Target Workflow.
- [ ] **Test Coverage**: Provide comprehensive unit tests covering standard behavior, edge cases, and failure modes. We expect >85% branch coverage for new modules.
- [ ] **Performance Validation**: Optimize the code for minimal overhead (e.g., Soroban gas/instructions limit, minimal latency in Actix, optimized asset loading in Next.js).
- [ ] **Regression Check**: No regressions introduced into the existing `{category}` test suites. Ensure `cargo test` or `npm run test` strictly passes.
- [ ] **Code Standards**: The code must precisely follow the established style guide (e.g., `rustfmt`, `clippy`, ESLint/Prettier) and include inline documentation for complex logic blocks.
...
def update_issues():
    issues = get_issues()
    print(f"Fetched {len(issues)} issues to update.")
    ...
    for issue in issues:
        ...
        body = generate_detailed_body(title, category)
        
        try:
            subprocess.run([GH_PATH, "issue", "edit", str(number), "--body", body], capture_output=True, text=True, check=True)
            print(f"Updated Issue #{number}: {title}")
            count += 1
            if count % 10 == 0:
                time.sleep(2) # rate limit buffer
        except subprocess.CalledProcessError as e:
            print(f"Failed to update #{number}: {e.stderr}")
